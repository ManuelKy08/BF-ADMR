...
echo -e "\t${colors[rand1]} |__| |__||______| |_|   |_||___||_|  |__| |__| |__||__| |__||_______||___| |_|"
	echo -e "                 \t${bd}${g}╭${w} Author  ${b}:${c} ${ul}Risky.Manuel${n}          ${g}${n}"
	echo -e "                 \t${bd}${g}│${w} Version ${b}:${w} 1.9.2                          ${g}${n}"
	echo -e "                 \t${bd}${g}│${w} Code    ${b}:${w} Bash, python                   ${g}${n}"
	echo -e "     \t${bd}${g} ╭──────────────┴${w} Date    ${b}:${w} 18 05 2023                 ${g}${n}"
}
...
clear
banner
echo -ne "      \t${c}[${w}>${c}] ${w}Asup keun ngaran web na guys ${g}:${n} "
read web
if [[ -z $web ]]; then
	printf "\n"
	echo -e "${b}[${r}!${b}]${w} Salahh bloon  !!"
	exit 0
fi
echo -ne "      \t${c}[${w}>${c}] ${w}asupkeun.txt wordlist ${g}(${w}Default${g}:${w} wordlist.txt${g}) ${g}:${n} "
read wordlist
